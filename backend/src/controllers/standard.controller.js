...
async function standardize(req, res) {
  ...
  try {
    ...
    logger.error('Failed to standardize cost', { error: err.message, tenantId, productId });
...
async function recomputeAndVariance(req, res) {
  ...
  try {
    ...
    logger.error('Failed to recompute variance', { error: err.message, tenantId, productId });
